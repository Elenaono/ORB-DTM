...
//        std::cerr << "count:" << count << std::endl;
    ...
    {
//        vertices_.emplace_back(vertices);
...
//        std::cout << "new size:" << vertices_.size() << std::endl;
...
//                    std::cout << "Pushing bad triangle " << std::endl;
            ...
            for(const auto e : polygon)
                triangles_.push_back(TriangleType(e.p1, e.p2, *p));

        }

        edges_.clear();
//        std::cerr << "before insert " << edges_.size() << std::endl;
        for(const auto t : triangles_)
        {
            edges_.push_back(t.e1);
            edges_.push_back(t.e2);
...
//        std::cerr << "after insert " << edges_.size() << std::endl;
